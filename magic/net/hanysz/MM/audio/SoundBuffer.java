 ...
 *
 * latest update 10th January 2007
 */


import javax.sound.sampled.*;
import java.io.*; /* only needed for writeBuffer hack */
...
// constant declarations moved to file MMConstants.java
    ...
    public void writeBuffer(FileOutputStream f) throws IOException {
	/* a hack--dump contents to a file--needs some improvement! */
	copyBufferToBytes(START_OF_BYTES, bufferAsBytes, START_OF_BUFFER, bufferSizeInFrames);
/* oops, endianness is wrong... */
	// try {f.write(bufferAsBytes);} finally {};
	for (int i=0; i<bufferSizeInBytes; i+=2) {
	    f.write(bufferAsBytes[START_OF_BYTES+i+1]);
	    f.write(bufferAsBytes[START_OF_BYTES+i]);
	}
    }
